...
const CategoryModule = () => {
    ...
    const handleSave = async (itemData) => {
        ...
        } catch (err) { setError(`Error al guardar la ${title}.`); }
    };

    const handleDelete = async (id) => {
        if (window.confirm(`¿Está seguro de que desea eliminar esta ${title}?`)) {
            try {
                await api.remove(apiEndpoint, id);
                fetchData();
            } catch (err) { setError(`Error al eliminar la ${title}.`); }
        }
    };
    ...
    return (
        <Box>
            ...
            <Button variant="contained" onClick={() => { setSelectedItem(null); setIsFormOpen(true); }} sx={{ mb: 2 }}>{`Nueva ${title}`}</Button>
