...
const BankStatementManagement = () => {
    ...
    const fetchBankStatements = async () => {
        try {
            ...
            const statementList = Array.isArray(data) ? data : data.results;
            setBankStatements(statementList || []);
            setError(null);
        } catch (err) {
    ...
    const handleSave = async (formData, id = null) => {
        try {
            ...
            if (id) {
                // For updates, Django REST Framework handles multipart with PUT/PATCH
                        ...
                        <TableBody>
                            {bankStatements.map((statement) => (
                                <TableRow key={statement.id}>
                                    <TableCell>{statement.bank_name || statement.bank}</TableCell>
                                    <TableCell>{statement.statement_date}</TableCell>
                                    <TableCell>
                                        {statement.file ? (
                                            <a href={statement.file} target="_blank" rel="noopener noreferrer">
                                                <IconButton size="small"><DownloadIcon /></IconButton>
                                                {statement.file.split('/').pop()}
                                            </a>
                                        ) : 'N/A'}
                                    </TableCell>
                                    <TableCell>
                                        <IconButton onClick={() => handleOpenForm(statement)}><EditIcon /></IconButton>
                                        <IconButton onClick={() => handleDelete(statement.id)}><DeleteIcon /></IconButton>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
