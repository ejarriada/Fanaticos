...
const ColorModule = () => {
    ...
    const fields = [
        ...
        { name: 'hex_code', label: 'Código Hex' }
    ];
    const columns = [
        { key: 'name', label: 'Nombre' },
        { key: 'hex_code', label: 'Código Hex' }
    ];
    ...
    const fetchData = async () => {
        ...
        } catch (err) { setError(`Error al cargar ${title}es.`); } 
    ...
    return (
        <Box>
            <Typography variant="h5" gutterBottom>{`Gestión de ${title}es`}</Typography>
